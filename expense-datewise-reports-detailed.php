...
$ret='SELECT ExpenseDate,SUM(ExpenseCost) as totaldaily FROM `tblexpense`  where (ExpenseDate BETWEEN :fdate and :tdate) && (UserId=:userid) group by ExpenseDate';
$stat = $conn->prepare($ret);
$stat->execute($data);
$fet =$stat->fetchAll(PDO::FETCH_OBJ);
$cnt=1;
